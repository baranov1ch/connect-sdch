...
exports.encode = function(options, encoderOptions) {
  var toSend = options.toSend || exports.defaultToSend;
  var toEncode = options.toEncode || exports.defaultToEncode;
  options.encodingFilter = function (enc) {
    return enc === 'identity';
  };
  options.acceptable = ['sdch'];
  options.beforeEncoding = function (req, res) {
    var dict = toSend(req, getAvailableDictionaries(req));
    if (dict)
      res.setHeader('Get-Dictionary', dict.url);
  };
  options.createEncoder = function(req, res, method) {
    if (method !== 'sdch')
      return null;
