  ...
  return function(req, res, next) {
    ...
    function checkthreshold(len) {
      if (compress && len < threshold) {
        debug('size below threshold: ' + len);
        compress = false;
      }
    }
    ...
    onHeaders(res, function() {
      ...
      if (!method || method === 'identity') {
        nocompress('not acceptable. accepts: ' + accept + " method: " + method);
        return;
      }
...
DictionaryStorage.prototype.addDictionary = function(dict) {
  this._dicts[this._dicts.length] = dict;
  this._clientHashMap[dict.clientHash] = dict;
  this._urlMap[dict.url] = dict;
  this._pathMap[dict.path] = dict;
  if(dict.domain) {
    var domainArray = this._dictsByDomain[dict.domain];
    if ( domainArray ) {
      domainArray[domainArray.length] = dict;
    } else {
      this._dictsByDomain[dict.domain] = [dict];
    }
  }
};

DictionaryStorage.prototype.dictsByDomain = function(domain) {
  return this._dictsByDomain[domain] || [];
};


//returns the newest dictionary
DictionaryStorage.prototype.getByDomain = function(domain) {
  var dictsByDomain = this.dictsByDomain(domain);
    if(dictsByDomain) {
      return dictsByDomain[dictsByDomain.length - 1];
  }
  return undefined;
};
